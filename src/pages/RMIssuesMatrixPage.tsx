...
type IssueRow = { issue_date: string; raw_material_name: string; quantity_kg: number };

export default function RMIssuesMatrixPage() {
  ...
  async function fetchData() {
    ...
    const { data: d } = await supabase
      ...
      .select('issue_date, raw_material_name, quantity_kg')
      .gte('issue_date', start)
      .lte('issue_date', end);
    setData((d as IssueRow[]) || []);
    setUnsaved({});
    setLoading(false);
  }
  ...
  const matrix = useMemo(() => {
    ...
    for (const row of data) {
      ...
      const day = getDate(new Date(row.issue_date + 'T00:00:00'));
      if (!m[name]) m[name] = {};
  ...
  async function saveAll() {
    ...
    for (const [key, qty] of entries) {
      ...
      await supabase.from('rm_daily_issues').upsert(
        {
          issue_date: dateStr,
          raw_material_name: name,
          quantity_kg: qty,
        },
        { onConflict: 'issue_date,raw_material_name' }
      );
    }
    setUnsaved({});
    fetchData();
  ...
  function exportCSV() {
    ...
    a.href = url;
    a.download = `RM-Issues-${format(month, 'yyyy-MM')}.csv`;
    a.click();
  }
  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        <div>
          <h1 className="text-xl font-semibold text-gray-900">Monthly Issues Matrix</h1>
                        ...
                        <td key={day} className="px-1 py-1 text-center">
                          <input
                            ...
                            className={`w-full px-1 py-1 text-center border rounded text-[11px] focus:outline-none focus:ring-1 focus:ring-teal-500 tabular-nums ${
                              val > 0 ? 'bg-amber-50 border-amber-200 text-amber-700' : 'border-gray-100 text-gray-400'
                            }`}
                            placeholder="0"
                          />
                        </td>
