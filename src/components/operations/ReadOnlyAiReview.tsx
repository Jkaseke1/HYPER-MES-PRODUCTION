...
import { AlertTriangle, BrainCircuit, CheckCircle2, Clock3, RefreshCw, ShieldCheck } from 'lucide-react';
...
const toneClasses: Record<FindingTone, { icon: typeof AlertTriangle; iconClass: string; bg: string }> = {
  red: { icon: AlertTriangle, iconClass: 'text-rose-600', bg: 'bg-rose-50 border-rose-100' },
  amber: { icon: Clock3, iconClass: 'text-amber-600', bg: 'bg-amber-50 border-amber-100' },
  teal: { icon: CheckCircle2, iconClass: 'text-teal-600', bg: 'bg-teal-50 border-teal-100' },
};
    ...
    <section className="overflow-hidden rounded-lg border border-teal-200 bg-white shadow-sm">
      <div className="flex flex-col gap-3 border-b border-slate-100 bg-slate-50/80 px-4 py-3 sm:flex-row sm:items-center sm:justify-between">
        <div className="flex items-start gap-3">
          <div className="mt-0.5 rounded-lg bg-teal-100 p-2 text-teal-700"><BrainCircuit className="h-5 w-5" /></div>
          <div><div className="flex items-center gap-2"><h2 className="text-sm font-extrabold text-slate-900">AI Review</h2><span className="rounded-full border border-teal-200 bg-white px-2 py-0.5 text-[10px] font-bold uppercase tracking-wide text-teal-700">Read-only</span></div><p className="mt-0.5 text-xs text-slate-500">Automated checks across weighbridge, GRN, transfers, Buffer and Sage.</p></div>
        </div>
        <button type="button" onClick={review} disabled={loading} className="inline-flex items-center gap-2 self-start border border-slate-200 bg-white px-3 py-2 text-xs font-bold text-slate-700 hover:bg-slate-100 disabled:opacity-50" title="Refresh read-only review"><RefreshCw className={`h-3.5 w-3.5 ${loading ? 'animate-spin' : ''}`} />Refresh review</button>
      </div>
      <div className="space-y-2 p-3">
        {error ? <div className="flex items-center gap-2 border border-rose-200 bg-rose-50 px-3 py-3 text-xs font-semibold text-rose-800"><AlertTriangle className="h-4 w-4" />{error}</div> : loading ? <div className="flex items-center gap-2 px-2 py-5 text-xs text-slate-500"><RefreshCw className="h-4 w-4 animate-spin" />Reviewing the latest records...</div> : findings.map((finding) => { const tone = toneClasses[finding.tone]; const Icon = tone.icon; return <div key={finding.title} className={`flex flex-col gap-2 border px-3 py-2.5 sm:flex-row sm:items-center sm:justify-between ${tone.bg}`}><div className="flex items-start gap-2"><Icon className={`mt-0.5 h-4 w-4 shrink-0 ${tone.iconClass}`} /><div><p className="text-xs font-bold text-slate-800">{finding.title}</p><p className="mt-0.5 text-xs text-slate-600">{finding.detail}</p></div></div><Link to={finding.href} className="shrink-0 text-xs font-bold text-teal-700 hover:text-teal-900">{finding.label} <span aria-hidden="true">-&gt;</span></Link></div>; })}
      </div>
      <div className="flex items-center gap-2 border-t border-slate-100 px-4 py-2 text-[10px] text-slate-400"><ShieldCheck className="h-3.5 w-3.5 text-teal-600" />No records are changed by this review.{refreshedAt ? ` Last checked ${refreshedAt.toLocaleTimeString()}.` : ''}</div>
    </section>
  );
}
