-- Complete a missing Sage supplier mapping without reopening approved GRN lines.
...
BEGIN
  ...
  END IF;
  IF v_grn.supplier_id IS NULL THEN
    RAISE EXCEPTION 'This GRN has no linked supplier';
  END IF;
  ...
  IF EXISTS (
    ...
    WHERE reference_id = p_grn_id
      AND event_type = 'grn_confirmed'
  ...
  WHERE id = v_grn.supplier_id;

  INSERT INTO public.approval_history
    (entity_type, entity_id, action, previous_status, new_status, approved_by, comments)
  VALUES
    ('grn', p_grn_id, 'supplier_reference_corrected', v_grn.status, v_grn.status,
     auth.uid(), 'Sage supplier code completed: ' || v_code || '. Sage retry remains a separate action.');
